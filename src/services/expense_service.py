...
def add_expense(session:Session, date:str, category:str, amount:float, description:str) -> bool:
    try:
...
def update_expense(session:Session, expense_id: int, date: str, category: str, amount: float, description: str) -> bool:
    try:
        if category not in CATEGORIES:
                print(f"Errore: categoria '{category}' non valida. Categorie ammesse: {', '.join(CATEGORIES)}")
                return False
...
def get_expenses_by_date_range(engine:Engine, start_date:datetime, end_date:datetime) -> pd.DataFrame:
...
def import_expenses_from_dataframe(session:Session, df: pd.DataFrame) -> tuple:
    ...
    success_count = 0
    error_count = 0
    invalid_categories = set([row["category"] for _, row in df.iterrows() if row["category"] not in CATEGORIES])
    if invalid_categories:
        raise Exception(f"Trovate le seguenti categorie non valide: {invalid_categories}")
...
def get_overall_monthly_totals(session:Session, start_date, end_date, dialect='postgresql'):
    if dialect == 'postgresql':
        month_expr = func.to_char(Expense.date, 'YYYY-MM')
    else:
        month_expr = func.strftime('%Y-%m', Expense.date)
