...
from services.expense_service import CATEGORIES, get_expenses_by_month, get_category_spending
...
with col1:
    selected_year = st.selectbox("Anno",
                                    options=list(
...
with col2:
    selected_month = st.selectbox("Mese",
...
st.subheader(
    f"📅 Riepilogo {format_month_year(selected_year, selected_month)}")
...
if not monthly_expenses.empty:
    ...
    total_target = sum(v for v in targets.values()
                        if v > 0) if targets else 0
    exceeded_categories = []
    ...
    fig_pie = px.pie(
        ...
        names='category',
        title=
        f"Distribuzione Spese - {format_month_year(selected_year, selected_month)}"
    )
    ...
    if targets:
        ...
        for category in CATEGORIES:
            ...
            if target > 0 or spent > 0:
                ...
                status = "✅" if spent <= target and target > 0 else "⚠️" if target > 0 else "➖"
                ...
                comparison_data.append({
                    ...
                    f"€{spent:.2f}",
                    'Target':
                    f"€{target:.2f}" if target > 0 else "Non impostato",
                    'Percentuale':
                    f"{percentage:.1f}%" if target > 0 else "N/A",
                    'Status':
                    status
                })
    ...
    monthly_display['date'] = monthly_display['date'].dt.strftime(
        '%d/%m/%Y')
    monthly_display['amount'] = monthly_display['amount'].apply(
        lambda x: f"€{x:.2f}")
